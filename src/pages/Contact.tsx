...
export default function Contact() {
  ...
  const servicesList = ['Video Editing', 'Web Development', 'Graphic Design', 'Social Media Handling', 'Logo Design'];
  const budgetOptions = ['₹5K - 15K', '₹15K - 50K', '₹50K+', 'Not sure yet'];

  const variants = {
    enter: (direction: "next" | "back") => ({
        ...
        >
          ...
          <button 
            onClick={() => {
              ...
              setCurrentStep(1);
              setFormData({ services: [], budget: '', name: '', email: '', message: '' });
            }}
      ...
      case 1:
        return (
          <motion.div 
            ...
            className="w-full"
          >
            <h2 className="font-display text-4xl md:text-5xl uppercase tracking-tighter mb-4 text-foreground">What are you looking for?</h2>
            <p className="text-foreground opacity-60 text-lg mb-8">Select all that apply.</p>
              ...
              {servicesList.map(service => {
                ...
                return (
                  <button
                    ...
                    className={`px-6 py-4 rounded-full text-lg font-medium transition-all duration-300 ${isSelected ? 'bg-primary text-white' : 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-foreground hover:bg-foreground/5'}`}
                  >
                    {service}
                  </button>
                )
            ...
            <button 
              type="button"
              onClick={() => { setSlideDir("next"); setCurrentStep(2); }}
              disabled={formData.services.length === 0}
              className="w-full px-10 py-6 bg-foreground text-background font-display text-2xl tracking-widest uppercase rounded-2xl transition-all duration-300 flex items-center justify-between group overflow-hidden relative disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      case 2:
        return (
          <motion.div 
            ...
            className="w-full"
          >
            <h2 className="font-display text-4xl md:text-5xl uppercase tracking-tighter mb-4 text-foreground">What's your budget range?</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12 mt-8">
              {budgetOptions.map(option => {
                ...
                return (
                  <button
                    ...
                    className={`p-6 rounded-2xl text-xl font-medium transition-all duration-300 text-left ${isSelected ? 'bg-primary text-white shadow-lg' : 'bg-[var(--glass-bg)] border border-[var(--glass-border)] text-foreground hover:bg-foreground/5'}`}
                  >
                    {option}
                  </button>
                )
            ...
            <div className="flex gap-4">
              <button 
                type="button"
                onClick={() => { setSlideDir("back"); setCurrentStep(1); }}
                className="px-8 py-6 bg-[var(--glass-bg)] border border-[var(--glass-border)] text-foreground font-display text-xl tracking-widest uppercase rounded-2xl transition-all duration-300 hover:bg-foreground/5"
              >
                Back
              </button>
              <button 
                type="button"
                onClick={() => { setSlideDir("next"); setCurrentStep(3); }}
                disabled={!formData.budget}
                className="flex-1 px-8 py-6 bg-foreground text-background font-display text-2xl tracking-widest uppercase rounded-2xl transition-all duration-300 flex items-center justify-between group overflow-hidden relative disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span className="relative z-10">Next</span>
                <div className="relative z-10 w-12 h-12 rounded-full bg-background/10 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-colors duration-300">
                  <ArrowRight className="group-hover:translate-x-1 transition-transform" />
      ...
      case 3:
        return (
          <motion.form 
            ...
            className="w-full"
          >
            <h2 className="font-display text-4xl md:text-5xl uppercase tracking-tighter mb-4 text-foreground">Tell us about your project</h2>
            ...
            <div className="flex gap-4">
              <button 
                type="button"
                onClick={() => { setSlideDir("back"); setCurrentStep(2); }}
                className="px-8 py-6 bg-[var(--glass-bg)] border border-[var(--glass-border)] text-foreground font-display text-xl tracking-widest uppercase rounded-2xl transition-all duration-300 hover:bg-foreground/5"
              >
                Back
              </button>
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-background w-full">
      ...
      {/* Right Side - Form */}
      <div className="w-full lg:w-1/2 p-10 md:p-20 flex flex-col items-center justify-center relative">
        <div className="w-full max-w-xl min-h-[400px] flex flex-col justify-center">
        ...
        {/* Quick Contact Info */}
        <div className="w-full max-w-xl mt-20 pt-10 border-t border-[var(--glass-border)] grid grid-cols-1 sm:grid-cols-2 gap-8">
